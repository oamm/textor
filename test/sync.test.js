...
describe('Sync Command', () => {
  ...
  it('should not throw error when syncing a standalone feature (null route)', async () => {
    // 1. Setup a state with a null route section
    const state = {
      sections: [
        {
          name: "StandaloneFeature",
          route: null,
          featurePath: "src/features/standalone",
          extension: ".astro"
        }
      ],
      components: [],
      files: {
        "src/features/standalone/Standalone.astro": {
          kind: "feature",
          hash: "some-hash",
    ...
    // Ensure the feature file exists on disk with correct signature
    const featPath = path.join(TEST_DIR, "src/features/standalone/Standalone.astro");
    await mkdir(path.dirname(featPath), { recursive: true });
    await writeFile(featPath, "<!-- @generated by Textor -->", "utf-8");

    await saveState(state);

    // 2. Run sync - this should not throw
