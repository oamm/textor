...
describe('Adopt Command', () => {
  ...
  it('should adopt a component by its name', async () => {
    ...
    expect(state.files['src/components/NamedComp/NamedComp.astro'].owner).toBe('NamedComp');
  ...
  it('should reconstruct sections with nested feature paths', async () => {
    await mkdir(path.join(TEST_DIR, 'src/pages/admin'), { recursive: true });
    await writeFile(path.join(TEST_DIR, 'src/pages/admin/settings.astro'), '<!-- @generated by Textor -->\n<div>Settings</div>', 'utf-8');
    
    await mkdir(path.join(TEST_DIR, 'src/features/admin/settings'), { recursive: true });
    await writeFile(path.join(TEST_DIR, 'src/features/admin/settings/Settings.astro'), '<!-- @generated by Textor -->\n<div>Settings Feat</div>', 'utf-8');

    await adoptCommand(null, { all: true });

    const state = JSON.parse(await readFile(path.join(TEST_DIR, '.textor', 'state.json'), 'utf-8'));
    const section = state.sections.find(s => s.route === '/admin/settings');
    expect(section).toBeDefined();
    expect(section.featurePath).toBe('src/features/admin/settings');
    expect(state.files['src/pages/admin/settings.astro'].owner).toBe('/admin/settings');
  });
